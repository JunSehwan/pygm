...
import {
  ...
  userLoadingEndwithNoone, setFriends,
  setFriendsDoneFalse
} from "slices/user";
import { setMyCouples } from 'slices/couple'
import { getAuth, onAuthStateChanged } from "firebase/auth";
import { doc, getDoc, onSnapshot } from "firebase/firestore";
import {
  db, getFriends, getMyCouples
  // getEducationsByUserId, getCareersByUserId, getSkillsByUserId
} from "firebaseConfig";
import LoadingPage from 'components/Common/Loading';
import Router from 'next/router';
import Board from 'components/Date/Board'
...
const index = () => {
  ...
  useEffect(() => {
    const authStateListener = onAuthStateChanged(auth, async (user) => {
      ...
      await getMyCouples().then((result) => {
        dispatch(setMyCouples(result));
      })
