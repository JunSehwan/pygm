...
import ProfileSetupFlow from "components/ProfileSetup/ProfileSetupFlow";
...
export default function ProfileSetupPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      try {
        ...
        console.error("[profile/setup] load error:", e);
        dispatch(userLoadingEndwithNoone());
        router.replace("/login");
      }
    });
  ...
  return (
    <>
      <Head>
        <title>필수정보 입력(1/2) | 차밍수프</title>
        <meta name="description" content="매칭을 위한 필수정보를 입력해주세요." />
      </Head>
