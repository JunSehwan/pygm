    ...
    <div>
      <div className='pb-4'>
        <div className='mx-auto px-2 text-left'>
          ...
          <div className='my-2 text-gray-600 leading-4'>
            매력적인 본인의 사진을 올려서 이성에게 어필해주세요!
            <p className='text-sm text-blue-600'>(최소 2장, 최대 5장까지 업로드 가능)</p>
          </div>
              ...
              {user?.thumbimage && user?.thumbimage?.length !== 0 ? (
                <>
                  {user?.thumbimage?.length === 1 ?
                    <img
                      className="rounded-xl h-[240px] w-full bg-black object-cover"
                      alt="thumbimg"
                    ...
                    (user?.thumbimage?.length > 1 &&
                      ...
                      user?.thumbimage?.map((v, index) => (
                        <img
                          className="rounded-xl h-[240px] w-full bg-black object-cover my-1"
                          alt="thumbimg"
                          key={index}
                ...
                <button onClick={openImageModal} className='bg-slate-100 rounded-xl h-[240px] w-full flex flex-col justify-center items-center'>
