...
function CompactActionBox({ nickname, onMoveCards, onMoveProfile }) {
  return (
    <div className="px-4 pb-5 pt-1">
      <div className="rounded-md border border-slate-200 bg-white px-4 py-4 shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-violet-50 text-violet-600">
            <PiBellRingingFill className="text-[16px]" />
          </div>
          <div className="min-w-0">
            <div className="text-[14px] font-extrabold text-slate-900">
              아직 도착한 호감은 없어요
            </div>
            <div className="mt-0.5 break-keep text-[12px] leading-5 text-slate-500">
              여성회원이 {nickname || "회원"}님에게 호감을 보내면 바로 안내드려요.
            </div>
          </div>
        </div>
        ...
        <div className="mt-3 grid grid-cols-2 gap-2">
          <button
            type="button"
            onClick={onMoveCards}
            className="flex h-11 items-center justify-center rounded-md bg-violet-600 text-[13px] font-bold text-white transition hover:bg-violet-700"
          ...
          <button
            ...
            className="flex h-11 items-center justify-center rounded-md bg-slate-900 text-[13px] font-bold text-white transition hover:bg-slate-800"
            style={{ cursor: "pointer" }}
          >
            프로필 보완
          </button>
...
export default function MaleEmptyState({
  ...
  return (
    <div className="flex min-h-full flex-col px-5 pt-5">
      ...
      <div className="mt-auto flex flex-row items-center gap-2 pb-4 pt-4">
        <button
          type="button"
          onClick={onMoveCards}
